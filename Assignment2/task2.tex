...
Figure X was implemented in $MultiSim$.
...
\subsection*{2.2}
  ...
  \subsection*{(i) Midband voltage gain}
    ...
    By using results from the AC analysis as seen in figure X to determine the midband gain in decibel and convert over to voltage gain($\frac{V}{V}$):

   	From figure X hence the midband gain in decibel is:
   	$$ V_{0} = 32.1848 dB $$ 
	...
	\begin{figure}[h]
        ...
        \begin{subfigure}[h]{0.25\textwidth}
                ...
                \label{fig:}
        \end{subfigure}
        \captionof{figure}{AC analysis of circuit in figure X}
	\end{figure}

	By adding a $R_{s}$ in the circuit as seen in figure X,  and find the midband gain of the circuit with AC analysis as seen in figure X:
	$$ 20 \cdot log(A_{M2}) = 25.2002 dB  \rightarrow A_{M3} = 18.19 \frac{V}{V} $$
	By using result from Task2.2.i that the midband gain without $R_{s}$ is $A_{M1} = 40.66 \frac{V}{V}$ and the formula for midband gain with $R_{s}$ would be:
	...
	By removing the load resistor and finding the midband gain in the circuit from AC analysis as seen in figure X.
